    ...
    <div className="space-y-5 max-w-2xl">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-xs mt-1" style={{ color: mutedColor }}>{notifications.filter(n => !n.isRead).length} unread</p>
        </div>
      ...
      <div className="space-y-2">
        {notifications.map((n, i) => (
          <motion.div key={n.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.05 }}
            ...
            style={{
              background: n.isRead ? 'transparent' : (isDark ? 'rgba(139,92,246,0.05)' : 'rgba(139,92,246,0.04)'),
              border: `1px solid ${n.isRead ? 'transparent' : 'rgba(139,92,246,0.1)'}`,
            }}>
            <div className="w-9 h-9 rounded-xl flex items-center justify-center text-lg flex-shrink-0" style={{ background: `${typeColors[n.type]}15` }}>
              {typeIcons[n.type]}
            </div>
            ...
            {!n.isRead && <div className="w-2 h-2 rounded-full bg-purple-500 mt-2 flex-shrink-0" />}
          </motion.div>
        ))}
      </div>
    </div>
